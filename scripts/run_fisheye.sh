...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/run_fisheye.sh [options]

Options:
  -i, --input PATH               Input fisheye image path
                                 (default: image/input_fisheye.jpg)
  -o, --output PATH              Output image path
                                 (default: tmp_fisheye_down.jpg)
      --method NAME              Projection method
                                 (equidistant|orthographic|stereographic|equisolid)
                                 (default: equidistant)
...
INPUT_IMAGE="image/input_fisheye.jpg"
OUTPUT_IMAGE="tmp_fisheye_down.jpg"
METHOD="equidistant"
...
validate_method() {
  case "$1" in
    ...
    *)
      ...
      echo "Allowed: equidistant, orthographic, stereographic, equisolid" >&2
      exit 2
      ;;
  esac
}
